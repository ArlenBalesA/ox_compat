-- ### rsg-MENU TO OX_LIB COMPAT ### --
if not Config.Modules['rsg-menu'].active then return end
local function exportHandler(exportName, func)
    AddEventHandler(('__cfx_export_%s_%s'):format(Config.Modules['rsg-menu'].resource_name, exportName), function(setCB)
        setCB(func)
    end)
end
...
local function convert(menu)
    ...
    for _,button in pairs(menu) do
        ...
        if RSGCore.Shared.Items[button.icon] then icon = ("nui://%s/html/images/%s"):format(Config.InventoryName, RSGCore.Shared.Items[tostring(button.icon)].image) else icon = button.icon or nil end
